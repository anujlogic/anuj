...
class PagesController extends Controller
{
    ...
    public function propertieshow($slug)
    {
		...
		$url = "https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=$property->location_latitude,$property->location_longitude&key=".$key."&radius=3000&type=school";
		...
		if(!empty($res)){
			...
			for($i=0;$i<count($res_array);$i++){
				if(stripos($res_array[$i]['name'],'school') !== false ){
					...
					$distance = distanceLatLng($property->location_latitude,$property->location_longitude,$lat,$lng,'K');
					
					$nearby_schools[] = array('name'=>$res_array[$i]['name'],'distance'=>$distance);
				}
			}
		}

        return view('pages.properties.single', compact('property','rating','relatedproperty','videoembed','cities','nearby_schools'));
    }
