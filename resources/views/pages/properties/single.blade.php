...
<style>
	...
	.nearby-sel{background-color: #3f51b5;color: #fff;padding: 5px;font-size: 18px;}
        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col s12 m4">
                    <div>
                        <h4 class="left">&#8377;{{ numbersToWords(round($property->price)) }}</h4>
                ...
                <div class="col s12 m8">
                    ...
                    @else
                        <div class="single-image">
                            @if(file_exists('public/property/'.$property->image) && $property->image)
                                <img src="{{asset('public/property/'.$property->image)}}" alt="{{$property->title}}" class="imgresponsive">
                            @endif
                        </div>
                    @endif
					...
					<div>
                        <ul class="collection with-header">
							...
							<li class="collection-item"><b>Title:</b>  {{$property->title}}</li>
							<li class="collection-item"><b>Bedroom:</b>  {{$property->bedroom}}</li>
							<li class="collection-item"><b>Bathroom:</b>  {{$property->bathroom}}</li>
							<li class="collection-item"><b>Area:</b>  {{$property->area}} Sq Ft</li>
							<li class="collection-item"><b>Address:</b>  {{$property->address}}</li>
							<li class="collection-item"><b>City:</b>  {{$property->city}}</li>
							<li class="collection-item"><b>Purpose:</b>  {{$property->purpose}}</li>
							<li class="collection-item"><b>Type:</b>  {{$property->type}}</li>
                        </ul>
                    </div>
                            ...
                            <ul class="collection with-header">
                                ...
                                @foreach($property->features as $feature)
									@if($feature->feature_type == 'amenity')
										<li class="collection-item"> <i class="small material-icons left">check_circle_outline</i>{{$feature->name}}</li>
									@endif
                                @endforeach
                            </ul>
                        @endif
					...
					<div>
                        @if($property->features)
                            <ul class="collection with-header">
                                <li class="collection-header grey lighten-4"><h5 class="m-0">Furnishings</h5></li>
                                @foreach($property->features as $feature)
                    ...
                    <div class="card-no-box-shadow card">
                        ...
                        <div class="card-image">
                            @if(file_exists('public/property/'.$property->floor_plan) && $property->floor_plan)
                                <img src="{{asset('public/property/'.$property->floor_plan)}}" alt="{{$property->title}}" class="imgresponsive">
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="card-no-box-shadow card">
						...
						{{-- $property->nearby --}}
							<a href="javascript:;" class="nearby-sel nearby-link" onclick="displayNearby('school',this);">Schools</a>  &nbsp; &nbsp; &nbsp;
							<a href="javascript:;" class="nearby-link" onclick="displayNearby('hospital',this);">Hospitals  </a>  &nbsp; &nbsp; &nbsp;
							<a href="javascript:;" class="nearby-link" onclick="displayNearby('restaurant',this);">Restaurants  </a>  &nbsp; &nbsp; &nbsp;
							<a href="javascript:;" class="nearby-link" onclick="displayNearby('department_store',this);">Departmental Store </a>  &nbsp; &nbsp; &nbsp;
							<a href="javascript:;" class="nearby-link" onclick="displayNearby('park',this);">Park</a> 
							...
							<div id="nearby_div">
								<ul class="collection with-header">
									@for($i = 0; $i < count($nearby_schools); $i++)
										<li class="collection-item">{{ $nearby_schools[$i]['name'] }} &nbsp; - &nbsp; {{round($nearby_schools[$i]['distance'],2)}} KM</li>	
									@endfor	
								</ul>
							</div>
                        </div>
                                    ...
                                    @if($property->user)
                                        <div class="card horizontal card-no-shadow">
                                            <div class="card-image p-l-10 agent-image">
                                                <img src="{{asset('public/users/'.$property->user->image)}}" alt="{{ $property->user->username }}" class="imgresponsive">
                                            </div>
                                    ...
                                    <li class="collection-item p-0">
                                        <a href="{{ route('property.show',$property_related->id) }}">
                                            <div class="card horizontal card-no-shadow m-0">
                                                ...
                                                <div class="card-image">
                                                    <img src="{{asset('public/property/'.$property_related->image)}}" alt="{{$property_related->title}}" class="imgresponsive">
                                                </div>
                                                @endif
                                                <div class="card-stacked">
                                                    <div class="p-l-10 p-r-10 indigo-text">
                                                        <h6 title="{{$property_related->title}}">{{ str_limit( $property_related->title, 18 ) }}</h6>
                                                        <strong>&#8377;{{numbersToWords(round($property_related->price))}}</strong>
                                                    </div>
                                                </div>
                                            </div>
                                        </a>
...
@section('scripts')
    ...
    <script src="{{ asset('public/frontend/js/jssor.slider.min.js') }}"></script>
    <script>
		...
		function displayNearby(nearby,elem){
			$('.nearby-link').removeClass('nearby-sel');
			$(elem).addClass('nearby-sel');
		
			var url = "{{action('AjaxController@getNearbyLocations')}}";
			
			$.ajax({
				type:'POST',
				url:url,
				data:'_token={{csrf_token()}}&nearby='+nearby+"&id={{$property->id}}",
				success:function(data) {//alert(data.data);
					//$("#city_id").html(data.city_options_str);
					$("#nearby_div").html(data.data_str);
				}
			});
		}
    ...
    <script>
        function initMap() {
            var propLatLng = {lat: <?php echo $property->location_latitude; ?>,lng: <?php echo $property->location_longitude; ?>};

            var map = new google.maps.Map(document.getElementById('map'), {zoom: 15,center: propLatLng});
    ...
    <script async defer src="https://maps.googleapis.com/maps/api/js?key=<?php echo env('GOOGLE_API_KEY'); ?>&callback=initMap"></script>
@endsection